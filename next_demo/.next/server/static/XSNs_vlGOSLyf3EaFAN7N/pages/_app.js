...
function makeStore(initialState, _ref) {
  ...
  initialState || store_reducer();

  if (isServer) {
    // 服务端渲染肯定会先于持久化状态加载
    return Object(external_redux_["createStore"])(store_reducer, initialState);
  } else {
    // redux-persist 来持久化存放在 Store 里的应用状态
    // 在服务端渲染时也可以初始化 Redux store 的状态，
    // 不过这里需要防止后续客户端加载的持久化状态覆盖掉服务端初始状态。(造成闪烁问题)
    var _require = __webpack_require__("VNb8"),
        persistReducer = _require.persistReducer,
        persistStore = _require.persistStore;

    var storage = __webpack_require__("T8f9").default;

    var persistedReducer = persistReducer({
      key: 'nextjs',
      storage: storage
    }, store_reducer);
    var store = Object(external_redux_["createStore"])(persistedReducer, initialState);
  ...
  _createClass(MyApp, [{
    ...
    value: function render() {
      ...
      return external_react_default.a.createElement(external_react_redux_["Provider"], {
        store: store
      }, external_react_default.a.createElement(Component, _extends({}, pageProps, router)));
    }
  }], [{
    key: "getInitialProps",
    value: function () {
...
/* harmony default export */ var _app = __webpack_exports__["default"] = (external_next_redux_wrapper_default()(makeStore)(_app_MyApp)); // import App, {Container} from 'next/app'
